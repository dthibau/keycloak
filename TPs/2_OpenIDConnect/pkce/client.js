...
function generateVerifier() {
    ...
    var verifier = generateRandomString(45)
    console.log('Verifier '+verifier);
    setOutput('output-verifier', verifier);
    setInput('input-verifier', verifier);
    sha256(verifier).then(hash => {
        console.log('Hash SHA-256 de la chaîne :', hash);
        var challenge = base64URLEncode(hash);

        setOutput('output-challenge', challenge);
        console.log('Challenge ' + challenge)
        setInput('input-challenge', challenge);
    });
...
function generateAuthenticationRequest() {
    ...
    var challenge = getInput('input-challenge');
...
function loadTokens() {
    ...
    var clientId = getInput('input-clientid');
    var verifier = getInput('input-verifier');
...
var steps = ['discovery', 'verifier', 'authentication', 'token', 'refresh', 'userinfo']
var state = loadState();
...
function step(step) {
    ...
    for (i = 0; i < steps.length; i++) {
        console.log(document.getElementById('step-' + steps[i]).style.display);
